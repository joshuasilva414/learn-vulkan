...
pub const GraphicsContext = struct {
    ...
    pub fn init(allocator: Allocator, app_name: [*:0]const u8, window: *glfw.Window) !GraphicsContext {
        ...
        const glfw_exts = glfw.getRequiredInstanceExtensions(&glfw_exts_count) orelse return error.GlfwMissingRequiredExtensions;
        try extension_names.appendSlice(allocator, glfw_exts[0..@as(usize, @intCast(glfw_exts_count))]);
...
fn createSurface(instance: Instance, window: *glfw.Window) !vk.SurfaceKHR {
    var surface_raw: glfw.VkSurfaceKHR = 0;
    if (glfw.createWindowSurface(@as(glfw.VkInstance, @intFromEnum(instance.handle)), window, null, &surface_raw) != .success) {
        return error.SurfaceInitFailed;
    }

    return @enumFromInt(surface_raw);
}
